...
import { getOAuthClientId, DRIVE_SCOPE, isClientIdConfigured } from "./config.js";
...
async function buildAuthUrl(interactive) {
...
export async function getToken(interactive) {
  if (!(await isClientIdConfigured())) {
    throw new Error("Google OAuth client ID not set — add it in the Drive section");
  }
  ...
  try {
    redirect = await chrome.identity.launchWebAuthFlow({
      url: await buildAuthUrl(interactive),
      interactive
    });
  } catch (e) {
